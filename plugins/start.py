    ...
    if id == owner_id:
        ...
        if "verify_" in message.text:
            ...
            if verify_status["link"] == "":
                reply_markup = None
            await message.reply(f"Your token successfully verified and valid for: 5 Hour", reply_markup=reply_markup, protect_content=False, quote=True)
            ...
            if IS_VERIFY and not verify_status['is_verified']:
                ...
                await message.reply(f"Your Ads token is expired, refresh your token and try again.\n\nToken Timeout: {get_exp_time(VERIFY_EXPIRE)}\n\nWhat is the token?\n\nThis is an ads token. If you pass 1 ad, you can use the bot for 5 Hour after passing the ad.", reply_markup=InlineKeyboardMarkup(btn), protect_content=False, quote=True)
